    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "8px" }}>
            <div style={{ position: "relative", width: "150px", height: "150px" }}>
                <svg width="150" height="150" viewBox="0 0 150 150" style={{ filter: `drop-shadow(0 0 12px ${color}40)` }}>
                    <circle cx="75" cy="75" r={radius} fill="none" stroke="#1e293b" strokeWidth="8" />
                    <circle
                ...
                <div style={{
                    ...
                    <span style={{ fontSize: "32px", fontWeight: "800", color, fontVariantNumeric: "tabular-nums", lineHeight: 1 }}>
                        {clamped.toFixed(1)}
                    </span>
                    <span style={{ fontSize: "11px", color: "#475569", marginTop: "2px" }}>/ 100</span>
                </div>
            </div>
            <span style={{ fontSize: "10px", letterSpacing: "0.14em", color: "#64748b", textTransform: "uppercase", fontWeight: "600" }}>
                Congestion Score
            </span>
        </div>
    );
    ...
    return (
        ...
        >
            <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                ...
                <span style={{ fontSize: "10px", color: "#64748b", textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: "600" }}>
                    {label}
                </span>
            </div>
            <span style={{ fontSize: "34px", fontWeight: "800", color: accent, fontVariantNumeric: "tabular-nums", lineHeight: 1 }}>
                {Math.round(animated)}
            </span>
        </div>
    );
...
function EventTypeCard({ item, selected, onClick }) {
    ...
    return (
        <button
            ...
            onMouseEnter={e => { if (!isActive) e.currentTarget.style.borderColor = `${item.color}80`; }}
            ...
            {PRIORITY_OPTIONS.map((p) => {
                ...
                return (
                    <button
                        ...
                        onMouseEnter={e => { if (!isActive) e.currentTarget.style.borderColor = `${p.color}60`; }}
                        onMouseLeave={e => { if (!isActive) e.currentTarget.style.borderColor = "#1e293b"; }}
                    >
                        {p.label}
                    </button>
...
function DurationSlider({ value, onChange }) {
    ...
    const getLabel = (v) => {
        if (v >= 60) return `${(v / 60).toFixed(1).replace(".0", "")}h ${v % 60 ? `${v % 60}m` : ""}`.trim();
        return `${v}m`;
    };
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            ...
            <div style={{ position: "relative", height: "6px", borderRadius: "3px", background: "#1e293b" }}>
                <div style={{
                    position: "absolute", left: 0, top: 0, height: "100%",
                    width: `${percentage}%`, borderRadius: "3px",
                    background: "linear-gradient(90deg, #0ea5e9, #6366f1)",
                    transition: "width 0.1s ease",
                }} />
            </div>
            <input
                type="range" min={15} max={480} step={15} value={value}
                onChange={(e) => onChange(Number(e.target.value))}
                style={{
                    width: "100%", marginTop: "-14px", position: "relative", zIndex: 1,
                    appearance: "none", WebkitAppearance: "none", background: "transparent",
                    cursor: "pointer", height: "20px", outline: "none",
                }}
            />
...
function HistoryItem({ item, index }) {
    const evt = EVENT_TYPES.find(e => e.label === item.event_type) || { icon: "📍", color: "#64748b" };
    const scoreColor = item.score >= 65 ? "#ef4444" : item.score >= 35 ? "#f59e0b" : "#22c55e";
...
export default function App() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const { data } = await axios.post(`${API_BASE}/api/forecast`, {
            ...
            setError(err.response?.data?.detail || err.response?.data?.error || "Forecast service unavailable.");
        } finally {
            setLoading(false);
        }
    };

    const selectedEvent = EVENT_TYPES.find(e => e.label === form.event_type) || EVENT_TYPES[0];
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                display: "grid", gridTemplateColumns: history.length > 0 ? "1fr 280px" : "1fr",
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
                    ...
                    {/* Header */}
                    <div style={{ borderBottom: "1px solid #1e293b", paddingBottom: "20px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "8px" }}>
                            ...
                            <span style={{ fontSize: "11px", color: "#22c55e", letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: "600" }}>
                                System Online · GridLock Engine v2
                            </span>
                        </div>
                        <h1 style={{ fontSize: "30px", fontWeight: "800", margin: 0, letterSpacing: "-0.03em", color: "#f8fafc" }}>
                            Event Impact Forecaster
                        </h1>
                        <p style={{ margin: "6px 0 0", fontSize: "14px", color: "#475569", lineHeight: "1.5" }}>
                            Predict congestion load and compute operational resource requirements in real time.
                        </p>
                    </div>

                    {/* Event Type Grid */}
                    <div>
                        <label style={{ fontSize: "11px", color: "#64748b", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: "600", display: "block", marginBottom: "10px" }}>
                            Event Type
                        </label>
                        <div style={{
                            display: "grid",
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "24px" }}>
                        <div style={{
                            ...
                            border: "1px solid #1e293b", borderRadius: "14px",
                            padding: "20px 22px",
                        }}>
                            <DurationSlider value={form.duration_minutes} onChange={handleDurationChange} />
                        </div>
                        <div style={{
                            background: "linear-gradient(135deg, #0a1628 0%, #0f172a 100%)",
                            border: "1px solid #1e293b", borderRadius: "14px",
                            ...
                            <label style={{ fontSize: "11px", color: "#64748b", textTransform: "uppercase", letterSpacing: "0.1em", fontWeight: "600", display: "block", marginBottom: "12px" }}>
                                Priority Level
                            </label>
                            <PriorityToggle value={form.priority} onChange={handlePriorityChange} />
                        </div>
                    </div>
                    ...
                    <button
                        ...
                        disabled={loading}
                        style={{
                            background: loading ? "#1e293b" : `linear-gradient(135deg, ${selectedEvent.color}, #6366f1)`,
                            ...
                            boxShadow: loading ? "none" : `0 4px 20px ${selectedEvent.color}30`,
                            display: "flex", alignItems: "center", gap: "10px",
                        }}
                        ...
                        {loading ? (
                            <>
                                <span style={{ display: "inline-flex", gap: "4px" }}>
                                    <span style={{ animation: "bounce 1.4s ease-in-out infinite", animationDelay: "0s" }}>●</span>
                                    <span style={{ animation: "bounce 1.4s ease-in-out infinite", animationDelay: "0.2s" }}>●</span>
                                    <span style={{ animation: "bounce 1.4s ease-in-out infinite", animationDelay: "0.4s" }}>●</span>
                                </span>
                                Forecasting
                            </>
                        ) : (
                    ...
                    {error && (
                        <div style={{
                            background: "rgba(239,68,68,0.06)", border: "1px solid #ef444450",
                        ...
                        <div style={{
                            ...
                            <div style={{
                                ...
                                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "20px" }}>
                                    <div>
                                        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "6px" }}>
                                            ...
                                            <p style={{ margin: 0, fontSize: "10px", color: "#64748b", textTransform: "uppercase", letterSpacing: "0.12em", fontWeight: "600" }}>
                                                Forecast Result
                                            </p>
                                        </div>
                                        <p style={{ margin: 0, fontSize: "16px", fontWeight: "700", color: "#e2e8f0" }}>
                                            {selectedEvent.icon} {form.event_type}
                                        </p>
                                        <p style={{ margin: "4px 0 0", fontSize: "12px", color: "#475569" }}>
                                            {form.duration_minutes} min · {PRIORITY_OPTIONS.find(p => p.value === form.priority)?.label} Priority
                                        </p>
                                    </div>
                                ...
                                <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
                                    <StatCard label="Manpower" value={result.recommended_manpower} accent="#38bdf8" icon="👷" />
                                    <StatCard label="Barricades" value={result.recommended_barricades} accent="#a78bfa" icon="🚧" />
                                </div>
